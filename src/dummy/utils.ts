...
export const createProductDummy = ({ id, storeId }: { id: number; storeId: number }) => {
...
export const createProductCategoriesDummy = ({ storeId }: { storeId: number }) => {
  const createProducts: Product[] = [];
  const num = 150;
  for (let x = 0; x < num; x += 1) {
    createProducts.push(createProductDummy({ id: productsDummy.length, storeId }));
  }
